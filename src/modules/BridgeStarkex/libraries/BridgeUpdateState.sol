...
import "../libraries/StarkExForcedActionState.sol";
import "../libraries/PublicInputOffsets.sol";
import "../libraries/StarkExConstants.sol";
import "../components/MessageRegistry.sol";
import "../components/AcceptModifications.sol";
...
PublicInputOffsets
{
  ...
  function up(uint256[] memory publicInput, uint256[] memory applicationData)
  external
  {
    require(
  ...
  returns (bytes32)
  {
    ...
    //require: programOutput does not contain all required fields.
    require(
      publicInput.length >=
  ...
  function performUpdateState(uint256[] memory publicInput, uint256[] memory applicationData)
  internal
  {
    rootUpdate(
  ...
  function performOnchainOperations(
    ...
    require(nModifications < 2**64, "In number of modifications.");
  ...
  */
  function verifyConditionalTransfers(
    ...
    for (uint256 i = 0; i < nCondTransfers; i++) {
      address factRegistryAddress = address(uint160(slidingAppData[offsetAppData]));
      ...
      //require: Condition for the conditional transfer was not met.
      require(
  ...
  function updateOnchainVaults(
    ...
    for (uint256 i = 0; i < nOnchainVaultUpdates; i++) {
      address ethAddress = address(uint160(slidingPublicInput[offsetPubInput]));
      uint256 assetId = slidingPublicInput[offsetPubInput + 1];
  ...
  */
  ...
  returns (uint256 consumedPubInputItems)
  {
    ...
    for (uint256 i = 0; i < nOnchainOrders; i++) {
      ...
      address orderSender = address(uint160(slidingPublicInput[offsetPubInput]));
